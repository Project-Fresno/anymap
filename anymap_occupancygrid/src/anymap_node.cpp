 ...
 * - sort the TF tree stuff, i.e. copy the current tf between base_link and
 * odom, and publish that till the next time the map is updated
 */
...
private:
  ...
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr
      lanes_processed_publisher;
  int lane_counter = 0;

  pcl::ConditionAnd<POINT_TYPE>::Ptr anymap_box_cond;
  pcl::ConditionalRemoval<POINT_TYPE> anymap_box_filter =
      pcl::ConditionalRemoval<POINT_TYPE>();

  rclcpp::Service<anymap_interfaces::srv::TriggerUpdate>::SharedPtr
      map_update_service;
  void update_anymap_callback(
      const std::shared_ptr<anymap_interfaces::srv::TriggerUpdate::Request>
          request,
      std::shared_ptr<anymap_interfaces::srv::TriggerUpdate::Response>
          response);
...
AnyMapNode::AnyMapNode() : Node("anymap_node") {
  ...
  *anymap_ptr.get() = anymap::init_anymap();
  ...
  anymap_publisher =
      this->create_publisher<nav_msgs::msg::OccupancyGrid>("anymap", 10);

  // initialize the service
  this->map_update_service =
      this->create_service<anymap_interfaces::srv::TriggerUpdate>(
          "~/trigger_update",
          std::bind(&AnyMapNode::update_anymap_callback, this,
                    std::placeholders::_1, std::placeholders::_2));

  // variables related to lanes layer
  rmw_qos_profile_t lanes_qos_profile = rmw_qos_profile_sensor_data;
  auto lanes_qos =
      rclcpp::QoS(rclcpp::QoSInitialization(lanes_qos_profile.history, 15),
                  lanes_qos_profile);
  lanes_subscription = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      "/lane_points", lanes_qos,
      std::bind(&AnyMapNode::lanes_callback, this, std::placeholders::_1));
  this->lanes_source_ptr =
      std::shared_ptr<observation_source::ObservationSource>(
  ...
  pcl::PointCloud<POINT_TYPE>::Ptr temp_lanes_cloud(
      new pcl::PointCloud<POINT_TYPE>());
  this->lanes_cloud = temp_lanes_cloud;
  this->lanes_processed_publisher =
      this->create_publisher<sensor_msgs::msg::PointCloud2>(
          "lanes_processed_points", 10);

  // variables related to the potholes layer
  rmw_qos_profile_t qos_profile = rmw_qos_profile_sensor_data;
  auto qos = rclcpp::QoS(rclcpp::QoSInitialization(qos_profile.history, 15),
                         qos_profile);
  potholes_subscription =
      this->create_subscription<sensor_msgs::msg::PointCloud2>(
          "/potholes_points", qos,
          std::bind(&AnyMapNode::potholes_callback, this,
                    std::placeholders::_1));
  this->potholes_source_ptr =
      std::shared_ptr<observation_source::ObservationSource>(
          new observation_source::ObservationSource("potholes",
                                                    this->anymap_ptr));
  this->potholes_postprocessor.set_layer_name("potholes");
  this->potholes_postprocessor.set_input_grid(this->anymap_ptr);
  pcl::PointCloud<POINT_TYPE>::Ptr temp_potholes_cloud(
      new pcl::PointCloud<POINT_TYPE>());
  this->potholes_cloud = temp_potholes_cloud;
  potholes_processed_publisher =
      this->create_publisher<sensor_msgs::msg::PointCloud2>(
          "potholes_processed_points", 10);

  // variables related to the lidar layer
  lidar_subscription = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      // "camera/depth/color/points", 15, std::bind(&AnyMapNode::lidar_callback,
      // this, std::placeholders::_1));
      "/scan_pcl", 15,
      std::bind(&AnyMapNode::lidar_callback, this, std::placeholders::_1));
  this->lidar_source_ptr =
      std::shared_ptr<observation_source::ObservationSource>(
  ...
  pcl::PointCloud<POINT_TYPE>::Ptr temp_lidar_cloud(
      new pcl::PointCloud<POINT_TYPE>());
  this->lidar_cloud = temp_lidar_cloud;
  this->lidar_processed_publisher =
      this->create_publisher<sensor_msgs::msg::PointCloud2>(
          "lidar_processed_points", 10);
  ...
  anymap_box_cond->addComparison(pcl::FieldComparison<POINT_TYPE>::Ptr(
      new pcl::FieldComparison<POINT_TYPE>("z", pcl::ComparisonOps::LT, 1.80)));
  anymap_box_cond->addComparison(pcl::FieldComparison<POINT_TYPE>::Ptr(
      new pcl::FieldComparison<POINT_TYPE>("z", pcl::ComparisonOps::GT, -0.1)));
  anymap_box_cond->addComparison(pcl::FieldComparison<POINT_TYPE>::Ptr(
      new pcl::FieldComparison<POINT_TYPE>("x", pcl::ComparisonOps::LT, 7.6)));
  anymap_box_cond->addComparison(pcl::FieldComparison<POINT_TYPE>::Ptr(
      new pcl::FieldComparison<POINT_TYPE>("x", pcl::ComparisonOps::GT, 0)));
  anymap_box_cond->addComparison(pcl::FieldComparison<POINT_TYPE>::Ptr(
      new pcl::FieldComparison<POINT_TYPE>("y", pcl::ComparisonOps::LT, 3.8)));
  anymap_box_cond->addComparison(pcl::FieldComparison<POINT_TYPE>::Ptr(
      new pcl::FieldComparison<POINT_TYPE>("y", pcl::ComparisonOps::GT, -3.8)));
  anymap_box_filter.setCondition(anymap_box_cond);

  this->grid_msg_ptr = std::shared_ptr<nav_msgs::msg::OccupancyGrid>(
      new nav_msgs::msg::OccupancyGrid);

  // TODO replace this with an action server that updates the map everytime it
  // is called
  this->timer_ = this->create_wall_timer(
      150ms, std::bind(&AnyMapNode::timer_callback, this));
  this->tf_timer = this->create_wall_timer(
      2ms, std::bind(&AnyMapNode::tf_publisher_callback, this));
}

void AnyMapNode::lidar_callback(
  ...
  pcl::PointCloud<POINT_TYPE>::Ptr transformed_cloud(
      new pcl::PointCloud<POINT_TYPE>());
...
void AnyMapNode::lanes_callback(
    const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
  this->lane_counter++;
  if (lane_counter != 7) {
    return;
  ...
  pcl::PointCloud<POINT_TYPE>::Ptr lanes_filtered(
      new pcl::PointCloud<POINT_TYPE>());
  ...
  rs_transform.rotate(
      Eigen::AngleAxisf(3.14159 / 2.0, Eigen::Vector3f::UnitY()));
  rs_transform.rotate(
      Eigen::AngleAxisf(-3.14159 / 2.0, Eigen::Vector3f::UnitZ()));

  rs_transform.rotate(
      Eigen::AngleAxisf(18.2 * 180.0 / 3.14159, Eigen::Vector3f::UnitX()));

  pcl::PointCloud<POINT_TYPE>::Ptr lanes_transformed_cloud(
      new pcl::PointCloud<POINT_TYPE>());
  pcl::transformPointCloud(*lanes_filtered, *lanes_transformed_cloud,
                           rs_transform);
  ...
  std::cout << "the lane layer image is of size "
            << lanes_postprocessor.image.size() << " "
            << lanes_postprocessor.image.channels() << std::endl;

  cv::Mat post_processor_uint8;
  this->lanes_postprocessor.image.convertTo(this->lanes_postprocessor.image,
                                            CV_8UC1, 255.0);
  this->lanes_postprocessor.image =
      lane_extension::process_lane_layer(this->lanes_postprocessor.image);
...
void AnyMapNode::potholes_callback(
    const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
  ...
  transform_y.rotate(
      Eigen::AngleAxisf(26.2 * 180.0 / 3.1415, Eigen::Vector3f::UnitY()));
  pcl::PointCloud<POINT_TYPE>::Ptr transformed_cloud(
      new pcl::PointCloud<POINT_TYPE>());
...
void AnyMapNode::update_anymap_callback(
    const std::shared_ptr<anymap_interfaces::srv::TriggerUpdate::Request>
        request,
...
void AnyMapNode::timer_callback() {
  ...
  if (this->anymap_ptr->exists("lidarProcessed")) {
    this->anymap_ptr->get("aggregate") +=
        this->anymap_ptr->get("lidarProcessed");
  }

  if (this->anymap_ptr->exists("potholesProcessed")) {
    this->anymap_ptr->get("aggregate") +=
        this->anymap_ptr->get("potholesProcessed");
  }

  if (this->anymap_ptr->exists("lanesProcessed")) {
    this->anymap_ptr->get("aggregate") +=
        this->anymap_ptr->get("lanesProcessed");
  }
  ...
  try {
    ...
    std::cout
        << "Could not lookup transform, is the localization thingi running?\n";
  }

  conv.toOccupancyGrid(*this->anymap_ptr.get(), "aggregate", 0, 1,
                       *this->grid_msg_ptr.get());
